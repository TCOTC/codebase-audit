...
- **D3d 跨仓发布物**：**独立发布**的同名联合类型/枚举（插件 SDK 类型包、客户端声明包）同属此类，但**必须先验证两侧消费域相同**
  （同一端点作用域、同一载荷），才可逐项比对——作用域不同的枚举，成员数差异可能是设计正确（见「已知误报」）。
  不变量优先取**单向子集** `published ⊆ source`，不要取相等：跨边界本就有合法的单向成员（仅内核内部使用、仅单向推送）。
  比对完成员后再比**配套载荷字段**，否则「只补名字」会产出说谎的类型。见模式 P17
...
| 用整数下标/位置编号做资源身份 | `Conf.Language(90)`、`window.siyuan.languages._kernel[142]` | **身份方案本身不是缺陷**：append-only ID 空间 + 同一发布内内核与前端读同一份物理文件，「同号同义」由文件自身保证；追加约定（`AGENTS.md`）与逐号回退英文都是有意设计，且实测从未出现「同号不同消息」。**只有同时满足「格式契约无载体」「契约无法表达副本的合法需求」「校验与被保护对象正交」三者才报告**（见 P16）；否则退化为「标识符不好看」的意见 |
| 端点作用域的载荷枚举 | `kernel/apicontract/block_transaction.go:104` 的 `api:"enum=..."` 只列 9 个 action，而内核 `switch op.Action` 有 96 个 | **不是同一语义集合，不可 diff**：该 enum 属于标题转换等已迁移端点的**载荷域**，`kernel/api/contract_block_transaction.go:15` 明确拒绝 AV 载荷（`operation.Srcs != nil \|\| len(operation.CellUpdates) > 0`），故故意不含 `setAttrView*`；且 `docs/API.md:139` 声明 `/api/transactions` 操作是内部实现、不承诺兼容性。成员数差异在这里是设计正确而非漏项。**只有「同一端点/同一作用域内的闭集」才可比**——先确认两侧的**消费域**相同，再比成员 |
...
| 2026-09-13 | 第九轮（架构层定向扫描）：新增模式 P16「位置编号做身份 + 格式契约无载体」与三件套诊断（载体 / 表达能力 / 校验）；新误报「用整数下标做资源身份」；API 契约层候选经查无缺口，排除；实证见 evidence.md；已提 issue #19430 |
| 2026-09-13 | 第十轮（架构层定向扫描）：新增模式 P17「跨仓公开类型契约的手工镜像无子集断言」；新误报「端点作用域的载荷枚举」（成员数差异可能是有意设计）；D3 增补「跨仓契约先验证消费域是否相同再比成员」；实证见 evidence.md |
