...
- **既有校验器**：`tsc` / `eslint` / 单测**都不看这层**。两个本 skill 的脚本只覆盖其中两项：
  ...
  - `scan_focus_coverage.py --root <src> --styles <scss> --styles <appearance>`
    覆盖**焦点可见性**（判据 I3 ⑤）。本仓库实测（第二十七轮，脚本修好后）：
    1186 个表单控件使用点中 1184 个有焦点指示、**2 个没有**（均为 `.protyle-toolbar__item`，
    即 #19499 第 2 点）。**第二十四轮的数字（641/462）已作废**——
    当时脚本不认识上游的 `:is()` 兜底、也不算特异性，两个方向都错。
    **六个判定前提都是实测写错过才对的**：SCSS 嵌套的后代语义、`transform` 也算指示、
    按完整类集合而非单类判定、兜底按真实标签匹配、`:is()`/`:where()` 的展开
    （且不得进入 `:not()` 内部）、特异性分属性比较。详见脚本 docstring 的陷阱表
  - 该脚本的输出分四节，**不要把后两节当缺口数**：确定无焦点指示 / 含祖先的抑制规则
    （祖先未知）/ 运行时赋类名中标签未知的 / 不带 class 的表单标签
...
- **取证陷阱**：
  ...
  - **本仓库的焦点可见性机制（判焦点缺口前必读）**：`util/_reset.scss` 对
    `button, input, select, textarea` **一律 `outline: none`**，
    而 `fa729c7c49`（#19493 的修复）之后又加了一条全局兜底
    `:is(button, input, select, textarea):where(:not(...)):focus-visible`（特异性 (0,1,1)）。
    所以「该组件没有 `:focus` 规则」**不再是**缺口信号（会吃到兜底）；真缺口是
    「该组件有一条**特异性更高**的 `outline: none` 把它反杀」
    （实例：`.protyle-toolbar__item:focus` (0,2,0)、`.protyle-preview__action button:focus` (0,2,1)）
  - **判定要展开 `:is()` / `:where()` 并算特异性，两件事缺一不可**：不展开会把已覆盖的报成缺口
    （实测 326 条假阳性），不比较特异性会把真缺口报成已覆盖（方向相反）
...
> 判据 I 里的 **I1「状态矩阵」**（空态 / 加载态 / 错误态 / 只读态）是判据清单中唯一没有实证的维度。
> 本节记录**人工对照的完整结论**：**本仓库当前没有找到确认实例**，因此**不建脚本、不立 P 条目**。
> 按 `SKILL.md` 的脚本准入标准，没有实例就建扫描器会重复「零产出脚本」的错误。

## 四种态的实际实现位置（这是做人工对照的前提）

**第一步必须先找到统一实现**，否则按关键字统计会得到完全错误的矩阵。本仓库实测：

| 状态 | 统一实现 | 位置 | 说明 |
|---|---|---|---|
| **空态** | `b3-list--empty` + `languages.emptyContent` | `util/Tree.ts:72`（`this.element.innerHTML` 里预置，`:229` 有对应点击判定） | **用 `Tree` 的面板自动有空态**，不需要面板自己写。按面板文件里搜 `empty` 会漏掉这一层 |
| **加载态** | ① `<img src="/stage/loading-pure.svg">`（44 处，分布最广）② 图标旋转 `fn__rotate`（如 `layout/dock/Tag.ts:213`/`:250`）③ `fn__loading` 类（少数） | 三种并存 | **`fn__loading` 不是通用信号**：按它统计会把「有加载态」判成「没有」。图标旋转那条尤其隐蔽——它加在刷新按钮的 svg 上 |
| **错误态** | `util/processMessage.ts:77`：`code < 0` 时 `showMessage` 并 `return false` → **回调不会被调用** | `util/fetch.ts:95` 的 `if (processMessage(response) && cb)` | 「回调里不检查 `code`」**不一定是缺陷**：失败时回调根本不会执行。`code > 0` 时返回 `response`（truthy）会调回调，但多数端点不会返回 >0 |
| **只读态** | **两种表示并存**：① 全局 `window.siyuan.config.readonly`（发布服务 / `--readonly`）② 编辑器级 `protyle.disabled`（`protyle/util/onGet.ts:503` 的 `disabledProtyle()` 设置） | 两者都要查 | 只按 `readonly` 关键字统计**一定会误判**（见下） |

## 本轮人工对照的结论（2026-09-14）：**未发现确认实例**

对照范围：桌面/移动的 **6 对兄弟实现**（书签、标签、大纲、文件树、反链、收件箱）
+ **26 个菜单文件**的只读守卫分布。逐项排除：

| 看起来像缺陷的差异 | 实际排除依据 |
|---|---|
| 移动端面板「空态 = 0」 | 空态由 `Tree` 统一提供，用 `Tree` 的面板不需自己写 |
| 移动端面板「加载态为 0」 | 用的是 `loading-pure.svg`（另一种实现），不是没有 |
| 桌面无加载态、移动有（书签、标签） | 桌面用图标旋转（`fn__rotate`）表示，属平台适配（桌面有刷新按钮） |
| `menus/block.ts` 的 `transferBlockRef` 无 `readonly` 守卫 | **调用点**有守卫：`protyle/gutter/index.ts:2910` 的 `if (!protyle.disabled)`。而且该端点的内核路由**已带 `CheckReadonly`**（`kernel/api/router.go`），即使到达也无数据风险 |
| 「回调里不检查 `code`」（如 `Tag.ts` 的 `this.data = response.data`） | `processMessage` 在失败时返回 `false` → 回调不执行 → 不会把 `undefined` 当数据用 |
| 标签面板桌面/移动的加载态实现不同 | 平台适配，非缺陷 |

**信号误判的两次实际踩坑（复用本节时务必先做这一步）**：

1. 用 `fn__loading` 作加载态信号 → 得到「6 个面板里 5 个没有加载态」；
   改用 `loading-pure.svg` 与 `fn__rotate` 后，绝大多数都有。
2. 用 `readonly` 作只读守卫信号 → 得到「`menus/block.ts` 完全没有只读守卫」；
   实际守卫是 `protyle.disabled`，**语义正确**。

**教训**：状态矩阵的「有没有」不能按类名或关键字统计，**必须先确定该状态的统一实现落在哪一层**
（面板 → Tree → 全局工具 → 内核），再问「谁绕过了它」。绕过统一实现才是信号。

## 如果要做机械化（前提是先拿到实例）

不要按「文件里有没有 `empty` / `loading` / `readonly` 关键字」统计。可行的方向是
**反向**的：先确定统一实现的入口（上表），再找「**有数据加载但没有走该入口**」的形态——
但那需要一份「哪些代码路径应当有状态」的清单，而这份清单本身只能由人给出，
所以在本仓库拿到实例之前，它仍然是文档而不是脚本。
